...
func (this *SceneMgr) CreateScene(s *netlib.Session, sceneId, sceneMode, sceneType, gameId int, platform string,
	params []int32, agentor, creator int32, replayCode string, hallId, groupId, totalOfGames int32,
	dbGameFree *server.DB_GameFree, bEnterAfterStart bool, paramsEx ...int32) *Scene {
	scene := NewScene(s, sceneId, sceneMode, sceneType, gameId, platform, params, agentor, creator, replayCode,
		hallId, groupId, totalOfGames, dbGameFree, bEnterAfterStart, paramsEx...)
	if scene == nil {
	...
	//
	key := this.makeKey(gameId, sceneMode)
...
func (this *SceneMgr) DestroyScene(sceneId int) {
	if scene, exist := this.scenes[sceneId]; exist {
		scene.OnStop()
		//
		key := this.makeKey(scene.GameId, scene.SceneMode)
		if ss, exist := this.scenesByGame[key]; exist {
