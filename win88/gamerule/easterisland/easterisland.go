...
func CalcLine(data []int, betLines []int64) (lines []LineData) {
	...
	for _, lineNum := range betLines {
		...
		if len(edata) == len(normalData) { //没有万能图案
			...
			for _, value := range normalData {
				...
				for i := 0; i < len(edata); i++ {
					if realData[0] == Element_EASTERISLAND { // WILD 不能替换 JACKPOT
...
func CaclScore(cards []int, betLines []int64) (int32, int32, int32, int32, bool) {
	...
	for _, line := range lines {
		...
		if line.Element == Element_EASTERISLAND && line.Count == LINE_CELL {
			isJackpot = true
		}
...
func CheckBigWin(slotsData []int) bool {
	if slotsData[0] == Element_WILD || slotsData[5] == Element_WILD || slotsData[10] == Element_WILD {
		return true
	}
	return false
}
...
Next:
	...
	for i := 0; i < ELEMENT_TOTAL; i++ {
		if s == SYMBOL1 {
			...
			if i == 0 || i == 5 || i == 10 {
				slotsData = append(slotsData, symbol1[rand.Intn(len(symbol1)-4)+4])
			} else {
