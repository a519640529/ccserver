...
func (this *ScenePolicyCaiShen) OnStart(s *base.Scene) {
	logger.Trace("(this *ScenePolicyCaiShen) OnStart, SceneId=", s.SceneId)
	sceneEx := NewCaiShenSceneData(s)
	if sceneEx != nil {
		if sceneEx.init() {
...
func (this *ScenePolicyCaiShen) OnStop(s *base.Scene) {
	logger.Trace("(this *ScenePolicyCaiShen) OnStop , SceneId=", s.SceneId)
...
func (this *ScenePolicyCaiShen) OnPlayerEnter(s *base.Scene, p *base.Player) {
	...
	logger.Trace("(this *ScenePolicyCaiShen) OnPlayerEnter, SceneId=", s.SceneId, " player=", p.SnId)
	if sceneEx, ok := s.ExtraData.(*CaiShenSceneData); ok {
		playerEx := &CaiShenPlayerData{Player: p}
...
func (this *ScenePolicyCaiShen) OnPlayerLeave(s *base.Scene, p *base.Player, reason int) {
	...
	logger.Trace("(this *ScenePolicyCaiShen) OnPlayerLeave, SceneId=", s.SceneId, " player=", p.SnId)
	if sceneEx, ok := s.ExtraData.(*CaiShenSceneData); ok {
...
func (this *ScenePolicyCaiShen) OnPlayerDropLine(s *base.Scene, p *base.Player) {
	...
	logger.Trace("(this *ScenePolicyCaiShen) OnPlayerDropLine, SceneId=", s.SceneId, " player=", p.SnId)
...
func (this *ScenePolicyCaiShen) OnPlayerRehold(s *base.Scene, p *base.Player) {
	...
	logger.Trace("(this *ScenePolicyCaiShen) OnPlayerRehold, SceneId=", s.SceneId, " player=", p.Name)
	//if sceneEx, ok := s.ExtraData.(*CaiShenSceneData); ok {
	//	if playerEx, ok := p.ExtraData.(*CaiShenPlayerData); ok {
	//发送房间信息给自己
...
func (this *ScenePolicyCaiShen) OnPlayerReturn(s *base.Scene, p *base.Player) {
	...
	logger.Trace("(this *ScenePolicyCaiShen) OnPlayerReturn, SceneId=", s.SceneId, " player=", p.Name)
	if sceneEx, ok := s.ExtraData.(*CaiShenSceneData); ok {
		if playerEx, ok := p.ExtraData.(*CaiShenPlayerData); ok {
			//发送房间信息给自己
...
func (this *SceneStateCaiShenStart) OnPlayerOp(s *base.Scene, p *base.Player, opcode int, params []int64) bool {
	...
	case CaiShenPlayerOpStart: //开始
		...
		if taxRate < 0 || taxRate > 10000 {
			logger.Tracef("CaiShenErrorTaxRate [%v][%v][%v][%v]", sceneEx.GetGameFreeId(), playerEx.SnId, playerEx.spinID, taxRate)
			taxRate = 500
		}
		//水池设置
		...
		//if baseRate >= 100 || baseRate <= 0 || ctroRate < 0 || ctroRate >= 1000 || baseRate+ctroRate > 9900 {
		//	logger.Tracef("CaiShenErrorBaseRate [%v][%v][%v][%v][%v]", sceneEx.GetGameFreeId(), playerEx.SnId, playerEx.spinID, baseRate, ctroRate)
		//	baseRate = 9700
		//	ctroRate = 200
		//}
		//jackpotRate := 10000 - (baseRate + ctroRate) //奖池系数
		jackpotRate := ctroRate //奖池系数
		logger.Tracef("CaiShenRates [%v][%v][%v][%v][%v][%v]", sceneEx.GetGameFreeId(), playerEx.SnId, playerEx.spinID, taxRate, baseRate, ctroRate)
	...
	WriteBlack:
		...
		if writeBlackTryTimes < 100 && playerEx.CheckBlackWriteList(spinRes.TotalPrizeLine+spinRes.TotalPrizeJackpot+spinRes.BonusGame.GetTotalPrizeValue() > totalBetValue) {
			...
			logger.Warnf("CaiShenWriteBlackTryTimesOver [%v][%v][%v][%v][%v]", sceneEx.GetGameFreeId(), playerEx.SnId, gamePoolCoin, playerEx.BlackLevel, playerEx.WhiteLevel)
		}
...
func CaiShenCheckAndSaveLog(sceneEx *CaiShenSceneData, playerEx *CaiShenPlayerData) {
	//统计金币变动
	//log1
	logger.Trace("CaiShenCheckAndSaveLog Save ", playerEx.SnId)
