...
func (this *CoinPoolManager) ReportCoinPoolRecEvent(gameFreeId, groupId int32, platform string, coin int64,
	...
	dbGameFree := srvdata.PBDB_GameFreeMgr.GetData(gameFreeId)
	if dbGameFree != nil && dbGameFree.GetSceneType() != -1 { //非试玩场
		if IsFishGame(int(dbGameFree.GetGameId())) {
			if !ignore {
				this.loopCount += 1
				if this.loopCount > MAXCOINPOOLLOOPCOUNT {
					this.loopCount -= MAXCOINPOOLLOOPCOUNT
					needSave = true
				} else {
					needSave = false
				}
	...
	if wgRcp != nil {
		...
		if setting, exist := this.CoinPoolSetting[key]; exist {
			...
			case 4:
				//奖池1
				value := int64(wgRcp.GetValue())
				logger.Logger.Infof("$$$$$$$$奖池1 %v 修改金币 %v.$$$$$$$$", key, value)
				jackpot := SlotsPoolMgr.GetSlotsPoolData(wgRcp.GetGameFreeId(), wgRcp.Platform)
				jk := new(SlotJackpotPool)
				if jackpot != nil {
					jk = jackpot.(*SlotJackpotPool)
				}
				jk.Small = value
				SlotsPoolMgr.SetPool(wgRcp.GetGameFreeId(), wgRcp.Platform, jk)
			case 5:
				//奖池2
				value := int64(wgRcp.GetValue())
				logger.Logger.Infof("$$$$$$$$奖池2 %v 修改金币 %v.$$$$$$$$", key, value)
				jackpot := SlotsPoolMgr.GetSlotsPoolData(wgRcp.GetGameFreeId(), wgRcp.Platform)
				jk := new(SlotJackpotPool)
				if jackpot != nil {
					jk = jackpot.(*SlotJackpotPool)
			...
			case 6:
				//奖池3
				value := int64(wgRcp.GetValue())
				logger.Logger.Infof("$$$$$$$$奖池3 %v 修改金币 %v.$$$$$$$$", key, value)
				jackpot := SlotsPoolMgr.GetSlotsPoolData(wgRcp.GetGameFreeId(), wgRcp.Platform)
				jk := new(SlotJackpotPool)
				if jackpot != nil {
					jk = jackpot.(*SlotJackpotPool)
			...
			case 7:
				//奖池4
				value := int64(wgRcp.GetValue())
				logger.Logger.Infof("$$$$$$$$奖池4 %v 修改金币 %v.$$$$$$$$", key, value)
				jackpot := SlotsPoolMgr.GetSlotsPoolData(wgRcp.GetGameFreeId(), wgRcp.Platform)
				jk := new(SlotJackpotPool)
				if jackpot != nil {
					jk = jackpot.(*SlotJackpotPool)
...
func (this *CoinPoolManager) GetCoinPoolSettingByGame(platform string, gameId, gameMode, groupId int32) (settings []*webapi.CoinPoolSetting) {
	...
	for _, id := range ids {
		...
		if setting != nil {
			jackpot := SlotsPoolMgr.GetSlotsPoolData(id, platform)
			jk := new(SlotJackpotPool)
			if jackpot != nil {
				jk = jackpot.(*SlotJackpotPool)
			}
			s := &webapi.CoinPoolSetting{
				...
				JackpotCoin1:     jk.Small,
				JackpotCoin2:     jk.Middle,
				JackpotCoin3:     jk.Big,
				JackpotCoin4:     jk.Grand,
			}
